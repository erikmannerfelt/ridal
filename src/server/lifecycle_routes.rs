...
pub async fn upload_dataset(
    ...
    let outcome = stream_to_file(body, &temporary, room).await;
    // Cleaned up on every path out of here, including the ones that return
    // early below, so a refused upload leaves nothing behind.
...
async fn stream_to_file(
    ...
    while let Some(chunk) = futures_util::StreamExt::next(&mut stream).await {
        ...
        if written > room && since_check >= CHUNK_CHECK_BYTES.min(written) {
            return Err(too_large(written, room, room));
        }
